...
# Commit
git commit -m "Initial commit: Variant Analytics Dashboard v2.0"
...
2. Use Refresh BQ button → data goes to staging
3. Use Refresh GCS button → dashboards update instantly!
4. Check GCS bucket for cache files:
   ```bash
...
## How Two-Stage Caching Works

```
🔄 Refresh BQ Button
├── Query BigQuery (10-30 seconds)
├── Save to STAGING cache (GCS)
├── Dashboards still show OLD data
└── Users not interrupted!

🔥 Refresh GCS Button
├── Copy STAGING → ACTIVE cache
├── Clear session state
├── Dashboards now show FRESH data
└── Instant switch!

⏰ Auto-Refresh (10:15 UTC Daily)
...
Edit `app/config.py`:
```python
AUTO_REFRESH_HOUR = 10   # UTC hour
AUTO_REFRESH_MINUTE = 15 # UTC minute
```
