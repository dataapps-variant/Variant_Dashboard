...
def get_plan_color(plan_name, plan_index_in_app=0):
    ...
    base_color = APP_COLORS.get(app_name, "#6B7280")  # Gray fallback
    
    # Shade pattern: base, lighter, darker, lighter, darker...
    shade_pattern = [
        0,      # Base color
        0.15,   # Slightly lighter
        -0.15,  # Slightly darker
        0.30,   # More lighter
        -0.30,  # More darker
        0.45,   # Much lighter
        -0.45,  # Much darker
...
def build_plan_color_map(plans):
    ...
    # Sort plans within each app for consistent ordering
