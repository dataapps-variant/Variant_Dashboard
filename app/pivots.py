...
def format_indian_number(num):
    """Format number in Indian thousands format (1,00,000)"""
...
def calculate_sparkline_data(row_values):
    """Calculate sparkline data points for a row"""
    # Filter out None values and get numeric values
    values = [v for v in row_values if v is not None and not pd.isna(v)]
    if not values:
        return []
    return values
...
def process_pivot_data_for_aggrid(pivot_data, selected_metrics):
    ...
    for app_name, plan_name in plan_combos:
        for metric in selected_metrics:
            ...
            # Add sparkline data (as JSON string for AG Grid)
            row["Trend"] = sparkline_values
...
def render_pivot_table(pivot_data, selected_metrics, title, table_id="pivot"):
    ...
    with col4:
        ...
        st.markdown(f'<a href="#" style="text-decoration: none;"><button style="padding: 8px 16px; border-radius: 4px; border: 1px solid {colors["border"]}; background: {colors["surface"]}; color: {colors["text_primary"]}; cursor: pointer;">⬆️ Top</button></a>', unsafe_allow_html=True)
    ...
    class SparklineRenderer {
        init(params) {
            ...
            const points = validValues.map((v, i) => {
                const x = padding + (i / (validValues.length - 1 || 1)) * (width - 2 * padding);
    ...
    gb.configure_column(
        ...
        rowGroup=True,  # Enable grouping by App
        hide=True  # Hide in grouped mode (shown as group header)
    )
    ...
    # Pagination for virtual scrolling effect
    ...
    custom_css = {
        ...
        ".ag-pinned-left-header, .ag-pinned-left-cols-container": {
            "border-right": f"2px solid {colors['border']}"
        }
    }
