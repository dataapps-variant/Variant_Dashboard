...
Users management and dashboard access control
"""
...
def render_admin_panel():
    ...
    st.markdown(f'''
    <h2 style="text-align: center; color: {colors['text_primary']}; font-size: 20px; margin: 0 0 40px 0;">
        Admin Panel
    </h2>
...
def render_users_section(colors):
    ...
    st.markdown(f'''
    <h3 style="color: {colors['text_primary']}; font-size: 16px; font-weight: 600; 
    ...
    if user_list:
        cols = st.columns(min(len(user_list), 6))
...
def render_edit_user_form(colors):
    ...
    with st.form(f"edit_form_{user_id}"):
        ...
        with col1:
            ...
            new_password = st.text_input("New Password (blank to keep)", type="password")
...
def render_dashboard_access_section(colors):
    ...
    <p style="color: {colors['text_secondary']}; font-size: 13px; margin-bottom: 16px;">
        <em>Note: Admin users have access to all dashboards.</em>
    </p>
    ''', unsafe_allow_html=True)
    ...
    with st.expander("➕ Add New User", expanded=False):
        with st.form("add_user_form"):
            ...
            if new_role == "readonly":
                ...
                new_dashboards = "all"
                st.info("Admin users have access to all dashboards.")
