...
FIXED: Simplified HTML rendering to avoid Streamlit rendering issues
"""
...
def render_loading_animation(progress=50):
    ...
    if logo_base64:
        filter_style = "invert(1)" if theme == "dark" else "invert(0)"
        logo_html = f'<img src="data:image/png;base64,{logo_base64}" style="width:120px;height:auto;opacity:{opacity};filter:{filter_style};transition:opacity 0.5s ease;" alt="Loading..."/>'
    else:
        logo_html = f'<div style="width:120px;height:120px;background:{colors["accent"]};border-radius:20px;opacity:{opacity};display:flex;align-items:center;justify-content:center;font-size:48px;font-weight:bold;color:white;transition:opacity 0.5s ease;">V</div>'
    
    st.markdown(
        f'<div style="position:fixed;top:0;left:0;right:0;bottom:0;background:{colors["background"]};display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;">{logo_html}</div>',
        unsafe_allow_html=True
    )
