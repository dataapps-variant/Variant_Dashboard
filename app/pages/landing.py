...
- Dashboard table (not cards): Dashboard Name, Last Refresh BQ, Last Refresh GCS
...
def render_landing_page():
    ...
    for dashboard in DASHBOARDS:
        ...
        # Only show refresh times for enabled dashboards
        bq_display = bq_refresh if is_enabled else "--"
        gcs_display = gcs_refresh if is_enabled else "--"
        
        table_rows += f'''
        <tr style="cursor: {cursor}; opacity: {opacity};" 
    ...
    # Navigation buttons (since HTML table clicks won't trigger Streamlit)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f'''
